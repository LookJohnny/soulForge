...
from ai_core.services.response_parser import PADValues, StructuredResponse, parse_llm_response
...
def _resolve_tts_params(
    parsed: StructuredResponse, emotion_engine, new_emotion: str | None, prompt_result: dict
) -> dict:
...
async def chat_preview(req: ChatPreviewRequest, request: Request):
    ...
    return ChatPreviewResponse(
        ...
        thought=parsed.thought or None,
...
async def chat_preview_stream(req: ChatPreviewRequest, request: Request):
    ...
    async def event_stream():
        ...
        try:
            ...
            yield _sse({"type": "text_replace", "text": dialogue})

            # ── Phase 3: Metadata events ──
            if parsed.action:
                yield _sse({"type": "action", "text": parsed.action})
            if parsed.thought:
                yield _sse({"type": "thought", "text": parsed.thought})
            ...
            yield _sse(
                {
                    "type": "emotion",
                    "emotion": new_emotion,
                    "pad": {"p": parsed.pad.p, "a": parsed.pad.a, "d": parsed.pad.d},
            ...
            if req.with_hardware:
                ...
                yield _sse({"type": "hardware", **hw.to_dict()})
                ...
                for i, sentence in enumerate(sentences):
                    try:
                        if use_pad_tts:
                            ...
                            tts_params = _resolve_tts_params(
                                parsed, emotion_engine, new_emotion, prompt_result
                            )
                            audio_data = await tts.synthesize_to_wav(
                                text=sentence,
                                voice=voice_id,
            ...
            yield _sse({"type": "error", "message": "服务暂时出错，请重试"})
