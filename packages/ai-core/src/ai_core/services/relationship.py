"""Relationship Evolution Engine — five-axis bond between character and user.

Ported (design, not code) from aikeya's companion engine and reshaped to
SoulForge's server-side brain:

Axes
    affection 0–1000 (10x resolution, the historical ``affinity`` column),
    trust / intimacy / comfort / respect 0–100, energy 0–100.

Stages (conjunctive gating — every listed requirement must hold)
    STRANGER → ACQUAINTANCE → FRIEND → CLOSE_FRIEND → ROMANTIC_INTEREST
    → DATING → COMMITTED → SOULMATE, plus the locked COMPANION stage.
    Romance stages additionally require completed events (Phase 4).

Per-turn update — "the app is the game master"
    ``baseline_impact`` computes a deterministic delta from what the user
    said (mood / depth / emotional / question) with a non-linear affection
    phase (honeymoon ×1.5, comfort ×1.0, deep bond ×0.7).  The LLM may
    *suggest* deltas; ``merge_updates`` clamps them relative to the baseline
    (±2× baseline, floor ±5/±3) and never lets it touch energy.

Wall-clock decay (per-axis clocks, applied lazily on read)
    energy refills over 6 h; affection decays proportionally after 48 h
    (≤50 per session); trust decays after 7 days (≤10); a 72 h absence
    tags the mood "想你了".  ``decay_clocks`` records the last decay stamp so
    repeated reads never double-decay.

App modes
    ``dating_sim`` (default) lets the relationship axes move; ``companion``
    freezes them (mood / energy still move) and pins the stage to COMPANION
    while remembering ``saved_stage`` so switching back loses nothing.

Legacy stage names FAMILIAR / BESTFRIEND are accepted everywhere and mapped to
FRIEND / CLOSE_FRIEND (the Postgres enum keeps the old values).

Schema fallback: on a database that has not run migration 005 the engine
sniffs the missing columns and degrades to the old affinity-only behaviour so
...
LEGACY_STAGE_ALIASES: dict[str, str] = {"FAMILIAR": "FRIEND", "BESTFRIEND": "CLOSE_FRIEND"}
...
STAGE_REQUIREMENTS: dict[str, dict[str, Any]] = {
    ...
    "CLOSE_FRIEND": {"affection": 300, "trust": 70, "comfort": 50, "days": 7, "interactions": 25},
    "ROMANTIC_INTEREST": {
        "affection": 450,
        "trust": 75,
        "intimacy": 30,
...
def meets_requirements(
    ...
    if req.get("interactions") and state.get("total_interactions", 0) < req["interactions"]:
        missing.append(
...
def baseline_impact(
    ...
    *,
    ...
    is_question = any(ch in text for ch in "?？") or text.rstrip().endswith(("吗", "呢", "么"))
...
def describe_axes(state: dict) -> dict[str, str]:
    ...
    return {
        ...
        "comfort": _band(
            state.get("comfort", 0),
            [(90, "完全自在"), (70, "很放松"), (50, "挺自在"), (25, "还在适应"), (0, "有点拘谨")],
        ),
        "energy": _band(
            state.get("energy", 100),
            [(80, "精力充沛"), (60, "状态不错"), (40, "一般"), (20, "有点累"), (0, "累坏了")],
        ),
    }
