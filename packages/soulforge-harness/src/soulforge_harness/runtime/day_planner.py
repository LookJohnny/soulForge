...
def generate_day_plan(
    ...
    blocks = [
        ...
        DayBlock(
            8 * 60,
            9 * 60,
            "cooking" if cooks_meals else "idle",
            "早餐时段",
            persona.agent_id,
        ),
        ...
        DayBlock(
            12 * 60,
            13 * 60,
            "cooking" if cooks_meals else "rest",
            "午餐与午休",
            persona.agent_id,
        ),
        ...
        DayBlock(
            22 * 60 + 30, 24 * 60, "rest", "熄灯休息/低功耗待机", persona.agent_id
        ),
    ]
    ...
    return DayPlan(
        agent_id=persona.agent_id, blocks=blocks, rationale=rationale, day=plan_day
    )
