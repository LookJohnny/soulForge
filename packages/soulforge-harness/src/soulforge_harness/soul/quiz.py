...
QUESTIONS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "options": [
            {
                "label": "希望有人立刻发现并问我怎么了",
                "scores": {"ANX": 0.6, "AVO": 0.0},
            },
...
def generate_identity(
    ...
    digest = hashlib.sha256(
        f"{seed}:{sorted(profile['prefs'].items())}:{key}".encode()
    ).digest()
    return {
...
def build_character(profile: dict[str, Any], seed: str = "") -> dict[str, Any]:
    ...
    trait_words.append(
        "安静" if ai["E"] < 0.45 else ("热络" if ai["E"] > 0.6 else "随和")
    )
    if ai["O"] >= 0.55:
    ...
    interests = []
    interests.append(
        "你今天过得怎么样" if behavior["proactive"] >= 0.5 else "不打扰的陪伴"
    )
    interests += (
        ["窗外的天气和光线", "你随口提过的小事"]
        if calm
        else ["新鲜好玩的点子", "把无聊的事变有趣"]
    )
    if ai["O"] >= 0.55:
