...
真机确认：**画面、声音、口型全部正常**，SoulForge 的 TTS 驱动 Vidu 渲染的脸。
链路是 ai-core TTS → ffmpeg 转 24kHz 单声道 s16le → 20ms 分帧 → Vidu WebSocket
→ Vidu 渲染 → Agora 频道 → 浏览器。

延迟问题已解决：曾经一句 6.8 秒的话要等 16.7 秒才开口，查下来是 Fish 的固定
往返开销（两个字也要 6.6 秒），换 DashScope 后短句 0.77 秒。详见 TTS 那节。

### 两套并存的代价

两个服务同时开着时，浏览器留在旧页面刷新一下就会建一个实时版会话——
本轮因此多烧了 63 credits，而且因为两版计费比例不同（组件版 1.0/秒、
实时版 1.5/秒），从账单反推才发现。

**同时只开一个**，或者给两者用不同的端口并在页面上标明版本。
