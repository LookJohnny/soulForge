...
from engine.server.protocol import (
    PROTOCOL_VERSION, ActionCommand, BodyHello, Event as WireEvent,
    Observation, PlanState, Tick, Welcome, decode, encode,
)
...
class SoulForgeRuntimeServer:
    def __init__(self, personas: list[Persona], *, start_minute: float = 20 * 60 + 47,
                 time_scale: float = 6.0, tick_hz: float = 1.0, llm=None,
        ...
        if not math.isfinite(start_minute) or start_minute < 0:
            raise ValueError(f"start_minute must be finite and >= 0, got {start_minute!r}")
        ...
        self.runtime = CompanionRuntime(
            personas, WorldState(sim_minute=start_minute), llm=self.llm,
    ...
    async def _tick_loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                await self._broadcast(encode(Tick(sim_minute=self.sim_minute,
                                                  clock=self.runtime.world.clock())))
            except Exception as exc:  # one bad tick must not kill the brain
                self.runtime.log(self.sim_minute, "*", "tick_error", {"error": str(exc)[:200]})
    ...
    async def _send_to_embodying_bodies(self, command: ActionCommand) -> None:
        for body in list(self.bodies.values()):
            ...
            outgoing.deadline = self.sim_minute + max(1.0, command.duration_s / 60.0 * self.time_scale + 2.0)
            template = TEMPLATE_REGISTRY.get(command.template_id) if command.template_id else None
            outgoing.interruptible = template.interruptible if template else True
            outgoing.safety_class = "physical" if manifest.is_fail_closed() else "expressive"
    ...
    def _plan_state(self, agent_id: str) -> PlanState:
        ...
        last = next((t.detail for t in reversed(self.runtime.trace)
        ...
        return PlanState(
            ...
            activities=[{
                "template_id": a.template_id, "start_min": a.start_min,
                "duration_min": a.duration_min, "params": a.params,
                "interruptible": a.interruptible,
            } for a in (hour.activities if hour else [])],
            day_blocks=[{
                "start_min": b.start_min, "end_min": b.end_min,
                "activity_key": b.activity_key, "intent": b.intent,
            } for b in day.blocks],
            last_decision=last,
        )
    ...
    async def _handle_observation(self, body: BodyConnection, obs: Observation) -> None:
        ...
        if command is None:
            self.runtime.log(self.sim_minute, obs.agent_id, "observation_rejected",
        ...
        if obs.agent_id != command.agent_id:
            self.runtime.log(self.sim_minute, command.agent_id, "observation_rejected",
                             {"reason": "agent_id mismatch", "claimed": obs.agent_id,
                              "actual": command.agent_id, "body": body.manifest.body_id})
            return
        agent_id = command.agent_id
        self.runtime.log(self.sim_minute, agent_id, "observation", {
            "status": obs.status, "step": command.name,
            "detail": obs.detail,
        })
        ...
        if obs.status == "failed" and command and command.template_id:
            ...
            if template:
                recovery = MicroAction(
                    name=template.recovery, template_id=command.template_id,
                    params={"recovering_from": command.name}, duration_s=2.0,
                )
                self.runtime.log(self.sim_minute, agent_id, "plan_change", {
                    "level": "recovery", "template": command.template_id,
                    "failed_step": command.name, "recovery_clip": template.recovery,
                })
    ...
    def _ingest_event(self, wire: WireEvent) -> None:
        try:
            ...
            self.runtime.log(self.sim_minute, wire.target_agent or "*", "event_dropped",
    ...
    async def _event_worker(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                self.runtime.log(self.sim_minute, event.target_agent or "*",
                                 "event_error", {"error": str(exc)[:200]})
    ...
    async def _serve_body(self, socket) -> None:
        ...
        if not isinstance(hello, BodyHello):
            ...
            return
        manifest = EmbodimentManifest.from_dict({**hello.manifest,
        ...
        await self._safe_send(socket, encode(Welcome(
            body_id=hello.body_id, accepted_agents=accepted,
        ...
        try:
            async for raw in socket:
                try:
                    ...
                    self.runtime.log(self.sim_minute, hello.body_id, "wire_error",
                                     {"reason": str(exc)[:120]})
    ...
    async def serve(self, host: str = "127.0.0.1", port: int = 8765) -> None:
        ...
        async with websockets.serve(self._handle_connection, host, port) as ws_server:
            ...
            print(f"SoulForge runtime server @ ws://{host}:{self.bound_port}  "
...
def main() -> None:
    ...
    server = SoulForgeRuntimeServer(default_personas(), start_minute=start_minute,
                                    time_scale=args.time_scale, tick_hz=args.tick_hz)
