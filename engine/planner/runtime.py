...
from engine.planner.models import (
    DayPlan, Event, HourPlan, ImpactLevel, MicroAction, MinuteAction,
    Persona, PlanDelta, VISION_EVENT_KINDS, WorldState,
)
...
_MEDIA_KEYS = {
    "media_ref", "image", "image_url", "audio", "audio_url", "video", "video_url",
    "frame", "raw", "frame_ref", "audio_ref", "video_ref", "screenshot", "pcm",
    "jpeg", "jpg", "png", "webp", "waveform", "blob", "attachment",
}
_MEDIA_KEY_PARTS = (
    "media", "image", "audio", "video", "frame", "screenshot", "waveform",
    "thumbnail", "pixel", "pcm", "jpeg", "webp", "blob",
)
_MEMORY_MAX_DEPTH = 6
...
def _is_media_key(key: str) -> bool:
    ...
    return normalized in _MEDIA_KEYS or any(part in normalized for part in _MEDIA_KEY_PARTS)
...
def _sanitize_memory_update(update: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in list(update.items())[:_MEMORY_MAX_ITEMS]:
        ...
        if sanitized is _DROP_MEMORY_VALUE:
            continue
        encoded = json.dumps({key: sanitized}, ensure_ascii=False, separators=(",", ":"))
...
def _trusted_confirmed_hazard(event: Event) -> bool:
    ...
    try:
        ...
        return False
    if (payload.get("severity") != "critical" or confidence < 0.75
            or required < 3 or hits < required):
        return False
...
class CompanionRuntime:
    def __init__(self, personas: list[Persona], world: WorldState | None = None,
                 llm=None, adapter: Adapter | None = None, trace_limit: int = 20000,
                 memory_store=None):
        if not personas:
        ...
        self.day_plans: dict[str, DayPlan] = {
            agent_id: generate_day_plan(persona, self.world) for agent_id, persona in self.personas.items()
        }
    ...
    # -- main loop ----------------------------------------------------------
    def tick(self, minute: float, consume_events: bool = True) -> dict[str, MinuteAction]:
        ...
        due = [e for e in list(self.event_queue) if e.t_min <= minute] if consume_events else []
        for event in due:
    ...
    def run(self, start_min: float, duration_min: float, step_min: float = 1.0) -> list[TraceEntry]:
        minute = start_min
    ...
    def _ensure_hour_plans(self, minute: float) -> None:
        ...
        for agent_id, persona in self.personas.items():
            if self.day_plans[agent_id].day != day:
                ...
                self.day_plans[agent_id] = generate_day_plan(persona, self.world, day=day)
                self._log(minute, agent_id, "plan_change",
                          {"level": "day", "new_day": day,
                           "rationale": self.day_plans[agent_id].rationale})
            plan = self.hour_plans.get(agent_id)
            if plan is None or plan.hour != hour:
                self.hour_plans[agent_id] = expand_hour(persona, self.day_plans[agent_id], hour, self.world)
                self._log(minute, agent_id, "plan_change",
                          {"level": "hour", "goal": self.hour_plans[agent_id].goal,
                           "activities": [a.template_id for a in self.hour_plans[agent_id].activities]})
    ...
    def _handle_event(self, event: Event, minute: float) -> None:
        self._log(minute, event.target_agent or "*", "event",
        ...
        if event.target_agent is not None and event.target_agent not in self.personas:
            self._log(minute, event.target_agent, "event_dropped",
        ...
        for agent_id in targets:
            ...
            decision = self.llm.decide(event, persona, self.world, current_template,
            ...
            if event.kind in VISION_EVENT_KINDS:
                if _trusted_confirmed_hazard(event):
                    ...
                    self._log(minute, agent_id, "decision", {
                        "impact": "LOW", "scope": "clamped",
                        "reason": ("unattested perception cannot exceed LOW; "
            ...
            if (event.payload.get("perception") and confidence is not None
                    and float(confidence) < 0.6
                    and decision.impact > ImpactLevel.LOW):
                self._log(minute, agent_id, "decision", {
                    "impact": "LOW", "scope": "clamped",
                    "reason": (f"perception confidence {confidence} below threshold: "
            ...
            if (activity is not None and not activity.interruptible
                ...
                self.push_event(Event(
                    t_min=safe_at, kind=event.kind, source=event.source, text=event.text,
                    payload={**event.payload, "_deferred_from": minute},
                    target_agent=agent_id,
                ))
                self._log(minute, agent_id, "decision", {
                    "impact": decision.impact.name, "scope": "deferred",
                    "reason": (f"activity {activity.template_id} is non-interruptible; "
            ...
            delta = self.replanner.apply(decision, event, persona,
                                         self.day_plans[agent_id], hour_plan, minute)
            self._apply_delta(agent_id, delta, minute)
            self._log(minute, agent_id, "decision", {
                "impact": delta.impact.name, "scope": delta.scope, "reason": delta.reason,
                "intent": decision.selected_intent, "emotional_read": decision.emotional_read,
    ...
    def _apply_delta(self, agent_id: str, delta: PlanDelta, minute: float) -> None:
        ...
        if delta.day_rewrite:
            ...
            self.hour_plans[agent_id] = expand_hour(
                self.personas[agent_id], self.day_plans[agent_id], int(minute // 60), self.world,
            )
            self._log(minute, agent_id, "plan_change",
                      {"level": "day", "rewritten": True,
    ...
    def _dispatch(self, agent_id: str, minute_action: MinuteAction) -> None:
        ...
        self._log(minute_action.minute, agent_id, "dispatch", {
            "template": minute_action.template_id,
            "steps": [s.name for s in minute_action.steps],
            "reason": minute_action.reason,
        })

    def _dispatch_micro(self, agent_id: str, action: MicroAction, minute: float) -> None:
        self.adapter(agent_id, action)
        kind = "dialogue" if action.dialogue else "dispatch"
        self._log(minute, agent_id, kind, {
            "step": action.name, "dialogue": action.dialogue,
            "gaze": action.gaze_target, "params": action.params,
        })

    def log(self, t_min: float, agent_id: str, kind: str, detail: dict[str, Any]) -> None:
        """Public trace hook for hosts (runtime server, adapters, test harnesses)."""
        self._trace_seq += 1
        self.trace.append(TraceEntry(t_min=t_min, agent_id=agent_id, kind=kind,
                                     detail=detail, seq=self._trace_seq))
