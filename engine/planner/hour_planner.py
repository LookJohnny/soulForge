...
from engine.planner.models import DayPlan, HourPlan, Persona, PlannedActivity, WorldState
...
_PHASING = {
    "cooking": [("cooking", 10, {"step": "prep_ingredients"}), ("cooking", 25, {"step": "stir_pan"}),
                ("cooking", 10, {"step": "wait"}), ("cooking", 10, {"step": "plate_up"}),
                ("chatting", 5, {"slot": "invite_user"})],
    "drawing": [("drawing", 40, {"step": "draw_stroke"}), ("rest", 5, {}), ("drawing", 15, {"step": "lean_back_review"})],
    "plant_care": [("plant_care", 20, {"step": "scan_leaves"}), ("plant_care", 20, {"step": "water_plant"}),
    ...
    "repair": [("repair", 45, {"step": "turn_wrench"}), ("cleaning", 15, {"zone": "workbench"})],
    "idle": [("idle", 60, {})],
}

_MOOD_DELTA = {
    "drawing": (0.15, 0.05), "cooking": (0.10, 0.10), "chatting": (0.20, 0.05),
    "plant_care": (0.08, 0.0), "rest": (0.10, -0.15), "repair": (0.05, 0.10),
    "study": (0.05, 0.05), "cleaning": (0.03, 0.05), "idle": (0.0, -0.05),
}


def _segments_for_hour(day_plan: DayPlan, hour_start_total: float, live_minute: float | None) -> list[tuple[float, float, str, str]]:
...
def expand_hour(persona: Persona, day_plan: DayPlan, hour: int, world: WorldState) -> HourPlan:
