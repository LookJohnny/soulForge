...
VISION_EVENT_KINDS = frozenset({
    EventKind.PERSON_DETECTED, EventKind.OBJECT_DETECTED, EventKind.GESTURE_DETECTED,
    EventKind.SCENE_CHANGED, EventKind.SOUND_EVENT, EventKind.PERCEPTION_ERROR,
})
...
class Persona:
    ...
    relationships: dict[str, float] = field(default_factory=dict)  # agent/user id -> 0..1
    daily_goals: list[str] = field(default_factory=list)
    meta: dict[str, Any] = field(default_factory=dict)   # config passthrough: voice, embodiment, dialogue style ...
...
class HourPlan:
    ...
    def activity_at(self, minute: float) -> PlannedActivity | None:
        for activity in self.activities:
            if activity.start_min <= minute < activity.start_min + activity.duration_min:
                return activity
        return None
...
class PlanDelta:
    ...
    param_updates: dict[str, Any] = field(default_factory=dict)  # template param patches
