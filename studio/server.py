...
from engine.planner import (
    CompanionRuntime, Event, EventKind, Persona, WorldState,
    load_characters, load_personas,
)
from engine.planner.llm_interface import MockBehaviorLLM, SafeDecisionLLM, build_llm
...
class StudioBrain:
    ...
    def _persona(self, agent_id: str, overrides: dict) -> Persona:
        ...
        if base is None:
            base = Persona(agent_id, overrides.get("name", agent_id.title()),
        ...
        persona = Persona(
            ...
            meta={**base.meta, "comfort_line": overrides.get("comfort_line")
    ...
    def _runtime_for(self, agent_id: str, overrides: dict) -> CompanionRuntime:
        ...
        if (agent_id in self.runtimes
                and self.fingerprints.get(agent_id) == fingerprint):
        ...
        runtime = CompanionRuntime(
            [persona], WorldState(sim_minute=self.sim_minute),
            llm=self.llm, memory_store=store,
    ...
    def chat(self, agent_id: str, text: str, overrides: dict) -> dict:
        ...
        runtime.handle_event_now(
            Event(t_min=self.sim_minute, kind=EventKind.USER_UTTERANCE,
                  source="user", text=text, target_agent=agent_id),
            self.sim_minute)
        dialogue = [a.dialogue for a in captured if a.dialogue]
        actions = [{"name": a.name, "correlation_id": a.correlation_id,
                    "params": {k: v for k, v in (a.params or {}).items()
                               if isinstance(v, (str, int, float, bool))}}
                   for a in captured]
        decision = next((t.detail for t in reversed(runtime.trace)
                         if t.kind == "decision"), {})
...
class RuntimeBodyClient:
    ...
    async def _connect(self):
        ...
        await socket.send(json.dumps({
            "type": "hello", "protocol": "0.2", "body_id": self.body_id,
            "backend": "voice", "agent_ids": agents,
            "manifest": {"supported_steps": ["speak_line", "look_at_user"],
                         "supported_templates": [],
    ...
    async def chat(self, agent_id: str, text: str) -> dict:
        async with self._lock:
            ...
            await socket.send(json.dumps({
                ...
                "text": text, "target_agent": agent_id, "payload": {},
            }, ensure_ascii=False))
            ...
            while loop.time() < (quiet_after or deadline):
                try:
                    ...
                    break
                data = json.loads(raw)
                if data.get("type") == "plan_state" and data.get("agent_id") == agent_id:
                    decision = data.get("last_decision") or decision
                if (data.get("type") != "action" or data.get("agent_id") != agent_id):
                    continue
                await socket.send(json.dumps({
                    "type": "observation", "command_id": data.get("command_id", ""),
                    "agent_id": agent_id, "status": "accepted",
                    "body_id": self.body_id}, ensure_ascii=False))
                actions.append({
                    "name": data.get("name"),
                    "correlation_id": data.get("correlation_id"),
                    "params": {k: v for k, v in (data.get("params") or {}).items()
                               if isinstance(v, (str, int, float, bool))},
                })
            ...
            return {"reply": " ".join(dialogue) or "（她想了想，没有说话。）",
                    "actions": actions, "decision": decision,
                    "relationship_user": None, "memory": {},
                    "llm": "RuntimeServer", "linked": True,
...
async def api_animations(_request: web.Request) -> web.Response:
    ...
    if directory.exists():
        for path in sorted(directory.glob("*.vrma")):
            out.append({"name": path.stem.replace("vrma_", "").replace("_", " "),
...
async def api_chat(request: web.Request) -> web.Response:
    ...
    if RUNTIME_CLIENT is not None:
        try:
            ...
            return web.json_response({"error": f"runtime link failed: {exc}"}, status=502)
    else:
        loop = asyncio.get_event_loop()
    ...
    try:
        if provider == "fish":
            try:
                ...
                audio = await tts_edge(text, {"id": "zh-CN-XiaoxiaoNeural",
                                              "rate": 0, "pitch": 0})
        else:
...
def _fish_request(text: str, voice: dict) -> bytes:
    ...
    payload = json.dumps({
        "text": text, "reference_id": voice.get("id"),
        "format": "mp3", "mp3_bitrate": 128, "normalize": True,
        "temperature": 0.7, "top_p": 0.8, "latency": "normal",
        ...
        try:
            request = urllib.request.Request(
                ...
                headers={"Content-Type": "application/json", "Connection": "close",
                         "Authorization": f"Bearer {FISH_KEY}", "model": FISH_MODEL})
...
async def _fish_via_curl(text: str, voice: dict) -> bytes:
    ...
    try:
        process = await asyncio.create_subprocess_exec(
            "curl", "-s", "-m", "30", "--retry", "2", "--retry-delay", "1",
            "-X", "POST", "https://api.fish.audio/v1/tts",
            "-H", "Content-Type: application/json",
            "-H", f"Authorization: Bearer {FISH_KEY}",
            "-H", f"model: {FISH_MODEL}",
            "-d", payload, "-o", str(out_path), "-w", "%{http_code}",
...
def main() -> None:
    ...
    print(f"SoulForge Studio → http://{args.host}:{args.port}  "
