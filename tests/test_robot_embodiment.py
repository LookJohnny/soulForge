...
def test_ir_to_executor_closed_loop_done():
    ...
    assert any(f["commands"] for f in backend.frames), "servo commands must actually reach the backend"
...
def test_stall_swallows_motion_but_reports_execution_metadata():
    ...
    assert all(not f["commands"] for f in fib.inner.frames), "stall must swallow all servo commands"
...
def test_illegal_joint_values_are_clamped_by_safety():
    ...
    leaked = [c["value"] for f in fib.inner.frames for c in f["commands"]
...
def test_watchdog_trips_and_enters_safe_pose():
    ...
    adapter, _ = make_adapter(watchdog_wall_s=5.0,
                              wall_clock=lambda: next(slow_wall, 200.0))
    obs = adapter.execute(command())
...
def test_planner_to_robot_end_to_end():
    ...
    for agent_id, action in dispatched[:4]:
        ir = ActionCommand(agent_id=agent_id, name=action.name,
                           template_id=action.template_id,
                           params=action.params, duration_s=action.duration_s)
        statuses.append(adapter.execute(ir).status)
    assert "done" in statuses
    assert all(s in ("done", "rejected") for s in statuses)
    assert any(f["commands"] for f in backend.frames), "planner intent produced real servo traffic"
