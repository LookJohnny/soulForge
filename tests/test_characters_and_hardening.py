...
def test_new_character_needs_no_code_changes(tmp_path):
    ...
    config = {
        ...
        "archetypes": {
            "default": {"focus": [["study", "自习"]], "evening": "chatting"}
        },
    }
    ...
    runtime = CompanionRuntime(
        personas, WorldState(sim_minute=19 * 60), llm=MockBehaviorLLM()
    )
    runtime.push_event(
        Event(
            t_min=19 * 60 + 1,
...
def make_runtime(**kwargs):
    ...
    return CompanionRuntime(
        personas, WorldState(sim_minute=19 * 60), llm=MockBehaviorLLM(), **kwargs
    )
...
def test_medium_patch_does_not_alias_decision_params():
    ...
    activity = next(
        a
        for a in runtime.hour_plans["kai"].activities
