...
def make_personas():
    return [
        ...
        Persona(
            ...
            "Kai",
            "steady_caretaker",
            traits=["reliable", "calm"],
            daily_goals=["做三餐", "把家里理顺"],
        ...
        Persona(
            ...
            daily_goals=["植物巡检", "环境监测"],
            relationships={"user": 0.7},
        ),
    ]
...
def test_hour_expansion_and_minute_resolution():
    ...
    assert minute.steps and minute.steps[0].adapter_command, (
        "minute action must map to adapter commands"
    )
...
def test_templates_declare_full_contract():
    ...
    assert not TEMPLATE_REGISTRY["cooking"].check_preconditions(
        ["pan", "stove"], "kitchen"
    )
...
def run_scenario(text, kind=EventKind.USER_UTTERANCE):
    runtime = CompanionRuntime(
        make_personas(), WorldState(sim_minute=19 * 60), llm=MockBehaviorLLM()
    )
    runtime.push_event(
        Event(
            t_min=19 * 60 + 2, kind=kind, source="user", text=text, target_agent="kai"
        )
    )
...
def test_medium_impact_preference_patches_template_params():
    ...
    cooking = [
        a for a in runtime.hour_plans["kai"].activities if a.template_id == "cooking"
    ]
    assert any(a.params.get("flavor") == "light" for a in cooking), (
        "cooking template must be re-parameterized"
    )
...
def test_high_impact_negative_emotion_rewrites_hour_to_companion_mode():
    ...
    assert runtime.personas["kai"].relationships["user"] > 0.75, (
        "comforting should strengthen relationship"
    )
