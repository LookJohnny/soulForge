...
async def start_server(**kwargs):
    server = SoulForgeRuntimeServer(
        ...
        time_scale=1.0,
        tick_hz=8.0,
        llm=kwargs.pop("llm", MockBehaviorLLM()),
        **kwargs,
    )
...
async def test_forged_agent_observation_is_rejected():
    ...
    try:
        ...
        command = next(
            f
            for f in frames
            if isinstance(f, ActionCommand) and f.template_id and f.agent_id == "kai"
        )
        ...
        recoveries = [
            t
            for t in server.runtime.trace
            if t.kind == "plan_change" and t.detail.get("level") == "recovery"
        ]
...
async def test_plan_state_only_reaches_subscribed_bodies():
    ...
    try:
        ...
        both_agents = {
            f.agent_id for f in both_frames if getattr(f, "type", "") == "plan_state"
        }
...
async def test_two_bodies_same_agent_receive_filtered_streams_and_ack_independently():
    ...
    try:
        ...
        robot, w2 = await connect_body(
            server.bound_port, "rover", ["kai"], robot_manifest, backend="robot"
        )
        ...
        assert [c.sequence for c in robot_cmds] == sorted(
            c.sequence for c in robot_cmds
        )
        ...
        await web.send(
            encode(
                Observation(
                    command_id=web_cmds[0].command_id,
                    agent_id="kai",
                    status="done",
                    body_id="web-1",
                )
        ...
        await robot.send(
            encode(
                Observation(
                    command_id=robot_cmds[0].command_id,
                    agent_id="kai",
                    status="done",
                    body_id="rover",
                )
...
def test_ir_v01_frames_still_decode_and_v02_roundtrips():
    ...
    full = ActionCommand(
        ...
        ttl_s=10,
        interruptible=False,
        safety_class="physical",
        ack_policy="full",
        trace_context={"span": "abc"},
        correlation_id="beat-3",
        target_body="rover",
    )
